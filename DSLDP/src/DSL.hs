...
doGenerateSimple :: [SynPop] -> IO ()
doGenerateSimple l = do
  let pops = zip l [0..]
  forM_ pops $ \(SynPop reg desc, n) -> do
    BLC.writeFile ("task_" ++ show n ++ ".json") $ encodePretty desc
  withFile "run_tasks.sh" WriteMode $ \h -> do
    BLC.hPut h $ BLC.pack $ "#!/bin/sh\n"
    BLC.hPut h $ BLC.pack $ "for I in {0.." ++ show (length pops - 1) ++ "}; do\n"
    BLC.hPut h $ BLC.pack $ "  python3 $DSLP_RUNTIME/dslp_runtime.py task_$I.json; done\n"
